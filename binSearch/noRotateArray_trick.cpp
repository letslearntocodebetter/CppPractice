...
int findRotationCount(const std::vector<int>& arr) {
    ...
    while (left <= right) {
        ...
        if (arr[mid] <= arr[right]) { // the arry is sorted from mid to right it means the min point should fall on the left side of the mid,
             so we change our right pointer to mid -1
            right = mid - 1;
